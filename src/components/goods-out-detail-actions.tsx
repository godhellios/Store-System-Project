...
type OrderLine = {
  product: { name: string; unit: { name: string } };
  quantity: number;
...
function buildWhatsAppUrl({
  ...
  const itemLines = lines.map((l, i) =>
    l.inputQty != null && l.inputUnit
      ? `${i + 1}. ${l.product.name} — ${l.inputQty} ${l.inputUnit} (= ${l.quantity} ${l.product.unit.name})`
      : `${i + 1}. ${l.product.name} — ${l.quantity} ${l.product.unit.name}`
  );
