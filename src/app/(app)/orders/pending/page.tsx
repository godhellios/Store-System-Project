    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 px-6 py-16 text-center">
          ...
          <div className="text-xs text-slate-400 mt-1">No orders awaiting approval</div>
        </div>
      ) : (
              ...
              <div key={o.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                ...
                <div className="flex items-start justify-between gap-3 px-4 py-3 border-b border-slate-100">
                  <div className="flex items-center gap-2 flex-wrap min-w-0">
                    <Link
                      href={`/orders/${o.id}`}
                      className="font-mono font-semibold text-blue-600 text-xs hover:underline flex-shrink-0"
                    >
                      {o.orderNumber}
                    </Link>
                    ...
                    {o.reference && (
                      <span className="text-[11px] text-slate-400 truncate">Ref: {o.reference}</span>
                    )}
                  </div>
                  <div className="text-right flex-shrink-0">
                    ...
                    {o.createdByName && (
                      <div className="text-[11px] text-slate-400">{o.createdByName}</div>
                    )}
                  </div>
                </div>
                ...
                <div className="px-4 pt-2.5 pb-1">
                        ...
                        <tr key={line.id} className="border-b border-slate-50 last:border-0">
                          ...
                          <td className="py-1.5 text-right font-semibold text-slate-800 whitespace-nowrap">
                            {line.inputQty != null ? (
                              <>
                                {line.inputQty}{" "}
                                <span className="font-normal text-slate-400">{line.inputUnit}</span>
                              </>
                            ) : (
                              <>
                                {line.quantity}{" "}
                                <span className="font-normal text-slate-400">{line.product.unit.name}</span>
                              </>
                            )}
                          </td>
                        </tr>
                  ...
                  {needsExpand && !isExpanded && (
                    <button
                      ...
                      className="text-[11px] text-blue-500 hover:underline pb-2"
                    >
                  ...
                  {needsExpand && isExpanded && (
                    <button
                      onClick={() => toggleExpand(o.id)}
                      className="text-[11px] text-slate-400 hover:underline pb-2"
                    >
                      Show less ▲
                    </button>
                  )}
                ...
                <div className="px-4 py-3 bg-slate-50 border-t border-slate-100">
                  <div className="text-[11px] text-slate-400 mb-2.5">
                    {o.lines.length} line{o.lines.length !== 1 ? "s" : ""} · {totalQty} units total
                    {" · "}
                    <span className="italic">{stockEffect(o)}</span>
                  </div>
                  {o.notes && (
                    <div className="text-[11px] text-slate-500 mb-2">
                      Note: <span className="italic">{o.notes}</span>
                    </div>
                  )}
                  ...
                  {isRejecting ? (
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => doAction(o.id, "reject", rejectNote)}
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <button
                        onClick={() => { setRejectingId(o.id); setRejectNote(""); }}
                        disabled={isProcessing}
                      ...
                      <Link
                        href={`/orders/${o.id}`}
                        className="text-xs px-3 py-1.5 border border-slate-300 rounded-lg text-slate-500 hover:bg-white transition-colors"
                      >
                        View detail →
                      </Link>
                    </div>
