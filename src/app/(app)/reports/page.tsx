...
type MovementRow = { id: string; quantity: number; type: string; createdAt: string; effectiveDate: string | null; product: { name: string; unit: { name: string } }; fromLocation: { name: string } | null; toLocation: { name: string } | null; order: { orderNumber: string } | null; orderId: string };
            ...
            {tab === 1 && (
              movData.length === 0 ? (
                ...
                <div key={m.id} className="bg-white rounded-xl border border-slate-200 px-4 py-3">
                  ...
                  <div className="text-xs text-slate-400 mt-1">{new Date(m.effectiveDate ?? m.createdAt).toLocaleString("id-ID", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jakarta" })}</div>
                </div>
              ))
            )}
          </div>
                ...
                <tbody>
                  {movData.length === 0 ? (
                    ...
                    <tr key={m.id} className="border-t border-slate-100 hover:bg-slate-50">
                      <td className="px-4 py-2.5 text-xs text-slate-500">{new Date(m.effectiveDate ?? m.createdAt).toLocaleString("id-ID", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jakarta" })}</td>
