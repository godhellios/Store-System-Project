  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-base font-semibold text-slate-800">{t("users.title", "User Management")}</h1>
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 border-b border-slate-200">
              <th className="px-4 py-2.5 text-left font-medium">{t("users.cols.name", "Name")}</th>
              <th className="px-4 py-2.5 text-left font-medium">{t("users.cols.email", "Email")}</th>
              <th className="px-4 py-2.5 text-left font-medium">{t("users.cols.role", "Role")}</th>
              <th className="px-4 py-2.5 text-left font-medium">{t("users.cols.status", "Status")}</th>
              <th className="px-4 py-2.5 text-left font-medium">{t("users.cols.created", "Created")}</th>
              <th className="px-4 py-2.5 text-left font-medium">{t("users.cols.actions", "Actions")}</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-400 text-xs">{t("common.loading", "Loading…")}</td></tr>
            ) : users.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-400 text-xs">{t("users.noUsers", "No users found")}</td></tr>
            ) : users.map((u) => (
              <tr key={u.id} className={`border-t border-slate-100 ${!u.isActive ? "opacity-60 bg-slate-50" : "hover:bg-slate-50"}`}>
                <td className="px-4 py-2.5">
                  <div className="font-medium text-slate-800">{u.name}</div>
                  {u.id === session?.user.id && (
                    <span className="text-[10px] text-blue-500 font-medium">{t("users.you", "You")}</span>
                  )}
                </td>
                ...
                <td className="px-4 py-2.5">
                  <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${u.isActive ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
                    {u.isActive ? t("users.active", "Active") : t("common.inactive", "Inactive")}
                  </span>
                </td>
                ...
                <td className="px-4 py-2.5 flex gap-3 items-center">
                  <button onClick={() => openEdit(u)} className="text-xs text-blue-600 hover:underline">{t("common.edit", "Edit")}</button>
                  <button onClick={() => toggleActive(u)}
                    className={`text-xs hover:underline ${u.isActive ? "text-red-500" : "text-green-600"}`}>
                    {u.isActive ? t("common.deactivate", "Deactivate") : t("common.activate", "Activate")}
                  </button>
                </td>
              </tr>
            ))}
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
              <h2 className="text-sm font-semibold text-slate-800">
                {modal === "add" ? t("users.modal.addTitle", "Add User") : `${t("common.edit", "Edit")} — ${editing?.name}`}
              </h2>
              <button onClick={() => setModal(null)} className="text-slate-400 hover:text-slate-600 text-lg leading-none">×</button>
            </div>
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">{t("users.modal.fullName", "Full Name *")}</label>
                <input value={form.name} onChange={(e) => setField("name", e.target.value)} required
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="e.g. John Doe" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">{t("users.modal.email", "Email *")}</label>
                <input type="email" value={form.email} onChange={(e) => setField("email", e.target.value)} required
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="user@example.com" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">{t("users.modal.role", "Role *")}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">
                  {modal === "add" ? t("users.modal.password", "Password *") : t("users.modal.newPassword", "New Password")}{modal === "edit" && <span className="text-slate-400 font-normal"> {t("users.modal.keepBlank", "(leave blank to keep current)")}</span>}
                </label>
                <input type="password" value={form.password} onChange={(e) => setField("password", e.target.value)}
                  required={modal === "add"} minLength={modal === "add" ? 6 : undefined}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder={t("users.modal.minChars", "Min. 6 characters")} />
              </div>
              {(modal === "add" || form.password) && (
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">{t("users.modal.confirmPassword", "Confirm Password *")}</label>
                  <input type="password" value={form.confirmPassword} onChange={(e) => setField("confirmPassword", e.target.value)}
                    ...
                    placeholder={t("users.modal.reEnter", "Re-enter password")} />
                </div>
              )}
              <div className="flex gap-3 pt-2">
                <button type="submit" disabled={saving}
                  ...
                  {saving ? t("common.saving", "Saving…") : modal === "add" ? t("users.modal.createUser", "Create User") : t("users.modal.saveChanges", "Save Changes")}
                </button>
